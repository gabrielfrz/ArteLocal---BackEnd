...
describe('User Controller', () => {
  ...
  describe('POST /user/register', () => {
    ...
    it('deve retornar erro 400 se faltarem campos', async () => {
      ...
      expect(res.body).toHaveProperty('message', 'Name, email, password, and role are required');
    });
    ...
    it('deve retornar erro 400 se o role for inválido', async () => {
      const res = await request(app)
        .post('/user/register')
        .send({
          name: 'Test User',
      ...
      expect(res.statusCode).toEqual(400);
      expect(res.body).toHaveProperty('message', 'Role must be either client or artisan');
    });
  });
